...
/**
 * name         - Bonjour service name / ioBroker namespace segment
 *
 * ip           - Resolved IPv4 address
 *
 * reqtype - Protocol supported by this device
 *
 * upnpClient  - Cached UPnP client instance
 *
 * pollCycle    - Incremented on each poll; used to throttle slow queries
 */
...
class Wiim extends utils.Adapter {
    ...
    async onReady() {
        ...
        this._refreshIntervalMs = normaliseRefreshInterval(Number(this.config.Refresh_Interval) || 10, this);
        ...
        bonjour.find({ type: 'linkplay' }, service => {
            ...
            const addrList = Array.isArray(service.addresses) ? service.addresses : `${service.addresses}`.split(',');
        ...
        discoveryTimeout = this.setTimeout(() => checkNewStreamers(this), DISCOVERY_INITIAL_DELAY_MS);
    }
    ...
    onStateChange(id, state) {
        ...
        switch (id) {
            ...
            case `${ns}leaveSyncGroup`:
                sendWiimCommand(this, 'ConnectMasterAp:JoinGroupMaster:eth0.0.0.0', ip, reqtype);
...
function normaliseRefreshInterval(configuredS, adapter) {
    if (configuredS > MAX_REFRESH_INTERVAL_S) {
        adapter.log.warn(`Refresh_Interval ${configuredS} s exceeds maximum – capped at ${MAX_REFRESH_INTERVAL_S} s`);
...
async function pollStreamer(adapter, streamer) {
    ...
    if (reqtype === 'https') {
        try {
            const { body } = await httpGet(reqtype, `${reqtype}://${ip}/httpapi.asp?command=getMetaInfo`);
            if (isUnloading) {
                return;
            }
    ...
    try {
        const { body } = await httpGet(reqtype, `${reqtype}://${ip}/httpapi.asp?command=getPlayerStatus`);
        if (isUnloading) {
            return;
        }
        const json = JSON.parse(body);
        ...
        adapter.setState(`${name}.lastRefresh`, new Date().toTimeString().substring(0, 8), true);
    ...
    // ── Schedule next poll ────────────────────────────────────────────────────
    if (!isUnloading) {
        const handle = adapter.setTimeout(() => pollStreamer(adapter, streamer), adapter._refreshIntervalMs);
...
async function pollDeviceInfo(adapter, streamer) {
    ...
    try {
        const { body } = await httpGet(reqtype, `${reqtype}://${ip}/httpapi.asp?command=getStatusEx`);
        if (isUnloading) {
            return;
        }
        const json = JSON.parse(body);
    ...
    try {
        const { body } = await httpGet(reqtype, `${reqtype}://${ip}/httpapi.asp?command=multiroom:getSlaveList`);
        if (isUnloading) {
            return;
        }
        const json = JSON.parse(body);
        adapter.setState(`${name}.multiroom_slaves`, Number(json.slaves) || 0, true);
        adapter.setState(`${name}.multiroom_slave_list`, JSON.stringify(json.slave_list || []), true);
    } catch (error) {
        if (!isUnloading) {
...
async function checkNewStreamers(adapter) {
    ...
    if (discoveryRunning) {
        discoveryTimeout = adapter.setTimeout(() => checkNewStreamers(adapter), DISCOVERY_INTERVAL_MS);
        return;
    }
...
async function createDataPoints(adapter, streamer) {
    ...
    httpGet(reqtype, `${reqtype}://${ip}/httpapi.asp?command=getStatusEx`)
        ...
        .catch(error => {
            adapter.log.debug(`Could not retrieve device name for ${name} @ ${ip}: ${error.message}`);
        });
}
